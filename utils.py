...
def load_dataset_train_test_split(name):
    assert name in ['20NG','AG','DBP','FDU','THU'], "name only supports '20NG','AG','DBP','FDU','THU', but your input is %s"%name
    ...
    if name == '20NG':
        ...
        other_group = ['alt.atheism', 'misc.forsale', 'soc.religion.christian']
        label_groups = [comp_group,rec_group,talk_group]
        return df, num_classes,label_groups, df1, df2
    if name == 'AG':
        num_classes = 4  # DBP:14 AG:4
        df1 = pd.read_csv('../datasets/AG_news/train.csv', header=None, index_col=None)
        df2 = pd.read_csv('../datasets/AG_news/test.csv', header=None, index_col=None)
...
def load_glove(filename='glove.6B.100d.txt'):
    with open(filename, 'r', encoding='utf-8') as f:
        ...
        for line in lines:
            ...
            word = line[0]
            vector = np.random.random(size=(embed_dim))
...
def load_subset_of_20NG(label_groups):
    num_classes = len(label_groups.keys())
    df1 = pd.read_csv('datasets/20NG/20ng-train-all-terms.csv')
    df2 = pd.read_csv('datasets/20NG/20ng-test-all-terms.csv')
    df = pd.concat([df1, df2])
    datas, labels = [], []
    for data, label in zip(list(df.content), list(df.label)):
        if label in label_groups.keys():
            datas.append(data)
            labels.append(label)
    res = pd.DataFrame(data={'content': datas, 'label': labels})
    return res, num_classes


if __name__ == '__main__':
    # df, num_classes,label_groups = load_dataset("AG")
    # print(num_classes)
