...
# log_txt_name = 'AG'  # AdamLR(0.01, lr_schedule={500: 1,3000: 0.1})  # 使用return_sequence=True, 然后全局池化 lr: 1e-4 lcm_stop: 30
# num_classes = 4
...
# df = pd.concat([df1, df2])
# epochs = 20   # 好像可以设置为20， 应该将学习率设置为1e-4
# epochs = 50  # 设置lr = 1e-3  lcm: 30, stop: 30
...
# df1.columns = ['label','title','content']
# df2.columns = ['label','title','content']
# df = pd.concat([df1,df2])       # lcm_stop: 6, 3 epochs=20
# epochs=20

# log_txt_name = 'Fudan'    # lcm_stop: 6, 6 epochs = 20
# num_classes = 20
# df = pd.read_csv('datasets/fudan_news.csv')
# epochs = 20

# log_txt_name = 'THU_log'   # epoch = 40, two_loss: 5 lcm:30
# df = pd.read_csv('datasets/thucnews_subset.csv')
# num_classes = 13
# epochs = 20

log_txt_name = log_txt_name + 'lstm_pre'
...
# English
w2c = load_glove('glove.6B.100d.txt')
wvdim = 100
...
embedding_matrix = create_embedding_matrix(w2c, vocab_size, wvdim, word_index)
...
for n in range(N):
    ...
    print('--Round ', n+1)
    # print('====Original:============')
    # basic_model = lstm.LSTM_Basic(maxlen,vocab_size,wvdim,hidden_size,num_classes,embedding_matrix)
    ...
    #
    # print('====LS:============')
    # ls_model = lstm.LSTM_LS(maxlen,vocab_size,wvdim,hidden_size,num_classes,embedding_matrix)
    ...
    #
    # print('====LCM:============')
    # dy_lcm_model = lstm.LSTM_LCM_dynamic(maxlen,vocab_size,wvdim,hidden_size,num_classes,alpha,None,embedding_matrix)
    ...
    # print('====LCM_word_level:============')
    # dy_lcm_model = lstm.LSTM_LCM_word_level(maxlen, vocab_size, wvdim, hidden_size, num_classes, alpha, None, None)
    # best_val_score, val_score_list, final_test_score, final_train_score = dy_lcm_model.train_val(data_package,batch_size, epochs,lcm_stop=30)
    # lcm_word_level_list.append(final_test_score)
    # with open('output/%s.txt' % log_txt_name, 'a') as f:
    #     print(n, 'lcm:', final_train_score, best_val_score, final_test_score, file=f)
    #
    print("====loss1:==========")
    loss_lcm = lstm.LSTM_two_loss(maxlen, vocab_size, wvdim, hidden_size, num_classes, alpha, None, embedding_matrix)
    best_val_score, val_score_list, final_test_score, final_train_score = loss_lcm.train_val(data_package, batch_size, epochs, lcm_stop=30)
    ...
    # inputs = [X_test, L_test]
    # outputs = loss_lcm.model.predict(inputs)
    # pred_probs = outputs[:, :num_classes]
    # pred = np.argmax(pred_probs, axis=-1)
    # isSame = [y_test[i] == pred[i] for i in range(len(y_test.tolist()))]
    # result = pd.DataFrame(data={'content': this_contents.tolist(), 'prob': pred_probs.tolist(), 'label': y_test.tolist(), 'isSame': isSame})
